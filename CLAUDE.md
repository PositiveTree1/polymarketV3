# Titan project

## Coding style
- Python 3.12+
- No unnecessary comments or docstrings unless asked
- Prefer implementation over explanation
- strongly typed
- no dict classes instead, dict are nuisance,we hate dict
- avoid default if unecessary, it is typed so we know what a class is made off

## Response style
- Be concise. No preamble.
- Don't summarise what you just did unless asked

## Architecture
- See titan_api_separation_plan.md for the full plan
- Check the implementation status table before starting any task